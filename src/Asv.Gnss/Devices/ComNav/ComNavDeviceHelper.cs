...
namespace Asv.Gnss
{
    public static class ComNavDeviceHelper
    {
        ...
        public static Task SetUnLogAllMessages(
        ...
        public static Task SetUnLogAllMessage(
            this IComNavDevice device,
            CancellationToken cancel = default
        )
        {
        ...
        public static async Task SetLogMessage(
            this IComNavDevice device,
            ComNavMessageEnum msg,
            ComNavTriggerEnum? trigger = null,
            uint? period = null,
            CancellationToken cancel = default
        )
        {
            ...
            var logList = await device
                .Pool<ComNavLogListAMessage, ComNavAsciiLogCommand>(
                    new ComNavAsciiLogCommand
                    {
                        Type = ComNavMessageEnum.LOGLIST,
        ...
        {
            return device.Push(
                new ComNavSetUnLockoutSystemCommand { SatelliteSystem = sys },
                cancel
            );
        }

        public static Task SetUnLockoutAllSystem(
            this IComNavDevice device,
            CancellationToken cancel = default
        )
        {
        ...
        public static async Task SetOnlyGpsAndGlonassSystem(
            this IComNavDevice device,
            CancellationToken cancel = default
        )
        {
            ...
            await device
                .SetLockoutSystem(ComNavSatelliteSystemEnum.BD2, cancel)
                .ConfigureAwait(false);
            await device
                .SetLockoutSystem(ComNavSatelliteSystemEnum.BD3, cancel)
                .ConfigureAwait(false);
            await device
                .SetLockoutSystem(ComNavSatelliteSystemEnum.GALILEO, cancel)
                .ConfigureAwait(false);
            await device
                .SetUnLockoutSystem(ComNavSatelliteSystemEnum.GPS, cancel)
                .ConfigureAwait(false);
            await device
            ...
            while (att <= 3)
            {
                ...
                var sys = (
                    await GetActiveGnssSystem(device, cancel).ConfigureAwait(false)
                ).ToArray();
        ...
        private static async Task<IEnumerable<ComNavSatelliteSystemEnum?>> GetActiveGnssSystem(
            this IComNavDevice device,
            CancellationToken cancel = default
        )
        {
            ...
            try
            {
                using var subscribeGsa = device
                    .Connection.Filter<Nmea0183MessageGSA>()
                    .Subscribe(_ =>
                    {
        ...
        public static Task SetPpsControl(
            this IComNavDevice device,
            ComNavPpsPolarityEnum polarity = ComNavPpsPolarityEnum.Negative,
            double period = 1.0,
            int pulseWidth = 1000,
            CancellationToken cancel = default
        )
        {
            return device.Push(
                new ComNavSetPpsControlCommand
                {
                    Polarity = polarity,
                    Period = period,
        ...
        public static async Task SetBaseStationSettings(
            this IComNavDevice device,
            double lat,
            double lon,
            double alt,
        ...
        )
        {
            ...
            await device
                .SetPpsControl(ComNavPpsPolarityEnum.Positive, 1.0, 10000, cancel)
                .ConfigureAwait(false);

            await device
                .SetLogMessage(ComNavMessageEnum.RTCM1, ComNavTriggerEnum.ONTIME, 1, cancel)
                .ConfigureAwait(false);
            await device
                .SetLogMessage(ComNavMessageEnum.RTCM31, ComNavTriggerEnum.ONTIME, 1, cancel)
                .ConfigureAwait(false);
            await device
                .SetLogMessage(
                    ComNavMessageEnum.RAWGPSSUBFRAME,
                    ComNavTriggerEnum.ONTIME,
                    1,
                    cancel: cancel
                )
                .ConfigureAwait(false);
            await device
                ...
                .ConfigureAwait(false);
            await device
                .SetLogMessage(ComNavMessageEnum.RTCM1004, ComNavTriggerEnum.ONTIME, 1, cancel)
                .ConfigureAwait(false);
            await device
                .SetLogMessage(ComNavMessageEnum.RANGE, ComNavTriggerEnum.ONTIME, 1, cancel)
                .ConfigureAwait(false);
            await device
                .SetLogMessage(ComNavMessageEnum.RTCM1012, ComNavTriggerEnum.ONTIME, 1, cancel)
                .ConfigureAwait(false);
            await device
                .SetLogMessage(ComNavMessageEnum.RTCM1019, ComNavTriggerEnum.ONTIME, 1, cancel)
                .ConfigureAwait(false);
            await device
                .SetLogMessage(ComNavMessageEnum.RTCM1020, ComNavTriggerEnum.ONTIME, 1, cancel)
                .ConfigureAwait(false);
            await device
        ...
        public static Task SetRoverSettings(
            this IComNavDevice device,
            CancellationToken cancel = default
        )
        {
