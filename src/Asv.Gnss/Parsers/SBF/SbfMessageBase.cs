...
namespace Asv.Gnss
{
    public abstract class SbfMessageBase:IProtocolMessage<ushort>
    {
        private ProtocolTags _tags = [];

        public string ProtocolId => SbfBinaryParser.GnssProtocolId;
        public ushort MessageId => (ushort)((MessageType & 0x1fff) + (MessageRevision << 13));
        public ushort Id => MessageId;
        public ProtocolInfo Protocol => SbfProtocol.Info;
        ...
        public void Deserialize(ref ReadOnlySpan<byte> buffer)
        {
            ...
            if (msgLength > origin.Length)
            {
                throw new GnssParserException(SbfBinaryParser.GnssProtocolId, $"Error to deserialize SBF packet message. Length too small (want [{msgLength}] read [{origin.Length}])");
            }

            var calculatedHash = SbfCrc16.checksum(origin.ToArray(), 4, msgLength - 4);
            if (calculatedHash != crc)
            {
                throw new GnssParserException(SbfBinaryParser.GnssProtocolId, $"Error to deserialize SBF packet message. CRC error (want [{crc}] calculated [{calculatedHash}])");
            }
        ...
        public string GetIdAsString() => MessageId.ToString();
