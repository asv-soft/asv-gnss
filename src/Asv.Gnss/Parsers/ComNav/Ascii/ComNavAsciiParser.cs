...
namespace Asv.Gnss
{
    public class ComNavAsciiParser : ProtocolParser<ComNavAsciiMessageBase, ComNavAsciiMessageId>
	{
		...
		private readonly ILogger<ComNavAsciiParser> _logger;
		...
		public ComNavAsciiParser(
			IProtocolMessageFactory<ComNavAsciiMessageBase, ComNavAsciiMessageId> messageFactory,
			IProtocolContext context,
			IStatisticHandler? statisticHandler) : base(messageFactory, context, statisticHandler)
		{
			_logger = context.LoggerFactory.CreateLogger<ComNavAsciiParser>();
		}

		public override ProtocolInfo Info => ComNavProtocol.AsciiInfo;
			...
			switch (_state)
			{
				...
				case State.CarriageReturn:
					...
					else
					{
						...
						try
						{
							InternalParsePacket(new ComNavAsciiMessageId(msgId), ref span);
						}
						catch (Exception ex)
						{
							_logger.ZLogTrace($"{ex.Message}[ComNavAsciiMessage{msgId}]");
