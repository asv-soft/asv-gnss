...
#if NETFRAMEWORK
                logMsg.Direction = Enum.TryParse(msg[0 + i * 6], true, out ComNavPortEnum dir)
                    ? dir
                    : ComNavPortEnum.NO_PORTS;
                logMsg.Message = Enum.TryParse(msg[1 + i * 6], true, out ComNavMessageEnum msgType)
                    ? msgType
                    : ComNavMessageEnum.UNKNOWN;
                logMsg.Format = Enum.TryParse(msg[2 + i * 6], true, out ComNavFormat format)
                    ? format
                    : ComNavFormat.None;
                logMsg.Trigger = Enum.TryParse(msg[3 + i * 6], true, out ComNavTriggerEnum trigger)
                    ? trigger
