...
namespace Asv.Gnss
{
    ...
    public abstract class ComNavAsciiMessageBase : ComNavStringMessageBase
    {
		...
		public override ProtocolInfo Protocol => ComNavProtocol.AsciiInfo;
		public override void Deserialize(ref ReadOnlySpan<byte> buffer)
		{
			var sync = BinSerialize.ReadByte(ref buffer);
			if (sync != ComNavAsciiParser.FirstSyncByte) throw new GnssParserException(ProtocolId, $"First sync byte error: want {ComNavAsciiParser.FirstSyncByte}, got {sync}");

			var originBuffer = buffer;

			var msg = Encoding.ASCII.GetString(buffer.ToArray(), 0, buffer.Length - 5)
				.Split(Encoding.ASCII.GetChars(new[] { ComNavAsciiParser.HeaderSeparator }));
			
			if (msg.Length != 2) throw new GnssParserException(ProtocolId, $"Error to deserialize {ProtocolId}:{MessageId} packet");

			var header = msg[0].Split(Encoding.ASCII.GetChars(new[] { ComNavAsciiParser.Separator }));
			var payload = msg[1].Split(Encoding.ASCII.GetChars(new[] { ComNavAsciiParser.Separator }));

			if (header.Length != 10) throw new GnssParserException(ProtocolId, $"Error to deserialize {ProtocolId}:{MessageId} packet");

			if (header[0] != MessageId) throw new GnssParserException(ProtocolId, $"Error to deserialize {ProtocolId} packet: message id not equal (want [{MessageId}] got [{header[0]}])");
#if NETFRAMEWORK
			Source = Enum.TryParse(header[1], true, out ComNavPortEnum source) ? source : ComNavPortEnum.NO_PORTS;
#else
			if (Enum.TryParse(typeof(ComNavPortEnum), header[1], true, out var source))
				Source = (ComNavPortEnum)source;
			else Source = ComNavPortEnum.NO_PORTS;
#endif
			var headerLength = header.Select(_ => _.Length).Sum() + header.Length - 1;
			MessageLength = payload.Length != 0 ? (payload.Select(_ => _.Length).Sum() + payload.Length - 1) : 0;

#if NETFRAMEWORK
			TimeStatus = Enum.TryParse(header[4], true, out ComNavTimeStatusEnum timeStatus) ? timeStatus : ComNavTimeStatusEnum.UNKNOWN;
#else
			if (Enum.TryParse(typeof(ComNavTimeStatusEnum), header[4], true, out var timeStatus))
				TimeStatus = (ComNavTimeStatusEnum)timeStatus;
			else TimeStatus = ComNavTimeStatusEnum.UNKNOWN;
#endif

			if (!uint.TryParse(header[5], out var gpsWeek))
				gpsWeek = 0;

			if (!double.TryParse(header[6], NumberStyles.Any, NumberFormatInfo.InvariantInfo, out var gpsSecs))
				gpsSecs = 0.0;
