...
namespace Asv.Gnss
{
    public class ComNavSimpleAnswerParser : ProtocolParser<ComNavSimpleAnswerMessageBase, ComNavAsciiMessageId>
	{
		public const string GnssProtocolId = "ComNavSimpleAnswer";
		public override ProtocolInfo Info => ComNavProtocol.SimpleAnswerInfo;
		private readonly ILogger<ComNavSimpleAnswerParser> _logger;

		public ComNavSimpleAnswerParser(
			IProtocolMessageFactory<ComNavSimpleAnswerMessageBase, ComNavAsciiMessageId> messageFactory,
			IProtocolContext context,
			IStatisticHandler? statisticHandler) : base(messageFactory, context, statisticHandler)
		{
			_logger = context.LoggerFactory.CreateLogger<ComNavSimpleAnswerParser>();
		}
		...
		public override bool Push(byte data)
		{
			foreach (var state in _states)
			{
				...
				try
				{
					InternalParsePacket(new ComNavAsciiMessageId(state.MessageId), ref span);
				}
				catch (Exception ex)
				{
					_logger.ZLogTrace($"{ex.Message}[ComNavSimpleAnswer{state.MessageId}]");
