...
namespace Asv.Gnss
{
    public class RtcmV3Message1020 : RtcmV3MessageBase
    {
        ...
        )
        {
            ...
            Acceleration.X =
                RtcmV3Helper.GetBitG(buffer, ref bitIndex, 5) * RtcmV3Helper.P2_30 * 1E3;
        ...
        /// <summary>
        /// Gets or sets iODE (0-6 bit of tb field). Временной интервал внутри текущих суток по шкале UTC(SU) + 03 ч 00 мин (tb - 03 ч 00 мин).
        /// </summary>
        public int Iode { get; set; }

        /// <summary>
        /// Gets or sets satellite frequency number.
        /// </summary>
        public int FrequencyNumber { get; set; }

        /// <summary>
        /// Gets or sets satellite health of operation.
        /// </summary>
        public int OperationHealth { get; set; }

        /// <summary>
        /// Gets or sets satellite accuracy of operation.
        /// </summary>
        public int OperationAccuracy { get; set; }

        /// <summary>
        /// Gets or sets satellite age of operation.
        /// </summary>
        public int OperationAge { get; set; }

        /// <summary>
        /// Gets or sets epoch of epherides (gpst). Временной интервал внутри текущих суток по шкале UTC(SU) + 03 ч 00 мин (tb).
        /// </summary>
        public DateTime EphemerisEpoch { get; set; }

        /// <summary>
        /// Gets or sets message frame time (gpst). Время начала кадра в рамках текущих суток  (tk).
        /// </summary>
        public DateTime FrameTime { get; set; }

        /// <summary>
        /// Gets satellite position (ecef) (m). Координаты n-го спутника в системе координат ПЗ-90 на момент времени tb.
        /// </summary>
        public EcefPoint Position { get; } = new EcefPoint();

        /// <summary>
        /// Gets satellite velocity (ecef) (m/s). составляющие вектора скорости n-го спутника в системе координат ПЗ-90 на момент.
        /// времени tb.
        /// </summary>
        public EcefPoint Velocity { get; } = new EcefPoint();

        /// <summary>
        /// Gets satellite acceleration (ecef) (m/s^2). Составляющие ускорения n-го спутника в системе координат ПЗ-90 на момент времени tb,
        /// обусловленные действием луны и солнца.
        /// </summary>
        public EcefPoint Acceleration { get; } = new EcefPoint();

        /// <summary>
        /// Gets or sets sV clock bias τn(tb) (s). Сдвиг шкалы времени n-го спутника tn относительно шкалы времени системы ГЛОНАСС
        ...
        /// <summary>
        /// Gets or sets delay between L1 and L2 Δτn (s). Временная разница между навигационным радиосигналом, переданным в поддиапазоне L2, и
