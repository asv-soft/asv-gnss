...
namespace Asv.Gnss
{
    ...
    public class GlonassWordOdd : GlonassWordBase
    {
        ...
        public override void Deserialize(byte[] data)
        {
            ...
            var bitIndex = 8U;
            omega = GlonassRawHelper.GetBitG(data, bitIndex, 16) * GlonassRawHelper.P2_15 * Math.PI; bitIndex += 16;
            ToaSec = GlonassRawHelper.GetBitU(data, bitIndex, 21) * GlonassRawHelper.P2_5; bitIndex += 21;
            DeltaT = GlonassRawHelper.GetBitG(data, bitIndex, 22) * GlonassRawHelper.P2_9; bitIndex += 22;
            DeltaDT = GlonassRawHelper.GetBitG(data, bitIndex, 7) * GlonassRawHelper.P2_14; bitIndex += 7;
            var h = (int)GlonassRawHelper.GetBitU(data, bitIndex, 5); bitIndex += 5;
            if (h is >= 25 and <= 31) h -= 32;
