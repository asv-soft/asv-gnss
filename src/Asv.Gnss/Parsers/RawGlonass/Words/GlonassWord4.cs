namespace Asv.Gnss
{
    ...
    public class GlonassWord4 : GlonassWordBase
    {
        ...
        public override void Deserialize(byte[] data)
        {
            ...
            bitIndex++;
            Accuracy = GlonassWord4.GetFt((byte)GlonassRawHelper.GetBitU(data, bitIndex, 4));
            bitIndex += 4 + 3;
        ...
        /// <summary>
        /// Gets or sets слово τn(tb) - сдвиг шкалы времени n-го НКА t n относительно шкалы времени системы ГЛОНАСС tc,
        ...
        /// <summary>
        /// Gets or sets смещение излучаемого навигационного радиосигнала поддиапазона L2 относительно навигационного
        /// радиосигнала поддиапазона L1 для n-го НКА.
        /// </summary>
        public double DelayTauN { get; set; }

        /// <summary>
        /// Gets or sets en - характеризует "возраст" оперативной информации, то есть интервал времени, прошедший от момента расчета (закладки)
        /// оперативной информации до момента времени tb для n-го НКА. Слово En формируется на борту НКА.
        /// </summary>
        public byte Age { get; set; }

        /// <summary>
        /// Gets or sets признак того, что на текущем интервале времени tb средствами ПКУ на НКА заложена (1)
        /// или не заложена (0) обновленная эфемеридная или частотно-временная информация.
        /// </summary>
        public byte P4 { get; set; }

        /// <summary>
        /// Gets or sets фактор точности измерений, характеризующий в виде эквивалентной ошибки прогнозируемую ошибку измерения
        ...
        /// <summary>
        /// Gets or sets текущая дата, календарный номер суток внутри четырехлетнего интервала, начиная с 1-го января
        ...
        /// <summary>
        /// Gets or sets номер НКА, излучающего данный навигационный сигнал (1) и соответствующий его рабочей точке
        /// внутри орбитальной группировки ГЛОНАСС.
        /// </summary>
        public byte Slot { get; set; }

        /// <summary>
        /// Gets or sets this method returns the time correction value (Ft) for a given navigation signal.
        ...
        private static double GetFt(byte ft)
        {
            switch (ft)
            {
                case 0:
