...
public static class RtcmV3Protocol
{
    public static ProtocolInfo Info { get; } = new("RTCMv3", "RTCMv3");
    
    public const byte SyncByte = 0xD3;

    public static ushort GetMessageId(byte[] buffer)
    {
        return (ushort)BitHelper.GetBitU(buffer, 24 /* preamble-8bit + reserved-6bit + length-10bit */, 12);
    }
    
    public static void RegisterRtcmV3Protocol(this IProtocolParserBuilder builder, Action<IProtocolMessageFactoryBuilder<RtcmV3MessageBase, ushort>>? configure = null)
    {
    ...
    public readonly static string[] ObsCodes =
    {
    ...
    public readonly static string[][] CodePris =
    {
    ...
    public static DateTime epoch2time(DateTime ep)
    {
        ...
        var days = (year - 1970) * 365 + (year - 1969) / 4 + doy[mon - 1] + day - 2 +
                   ((year % 4 == 0 && mon >= 3) ? 1 : 0);
    ...
    public static string GetRinexCodeFromMsm(NavigationSystemEnum sys, int signalId)
    {
        return sys switch
        {
            NavigationSystemEnum.SYS_GPS => RtcmV3Protocol.msm_sig_gps[signalId],
            NavigationSystemEnum.SYS_GLO => RtcmV3Protocol.msm_sig_glo[signalId],
            NavigationSystemEnum.SYS_GAL => RtcmV3Protocol.msm_sig_gal[signalId],
            NavigationSystemEnum.SYS_QZS => RtcmV3Protocol.msm_sig_qzs[signalId],
            NavigationSystemEnum.SYS_SBS => RtcmV3Protocol.msm_sig_sbs[signalId],
            NavigationSystemEnum.SYS_CMP => RtcmV3Protocol.msm_sig_cmp[signalId],
            NavigationSystemEnum.SYS_IRN => RtcmV3Protocol.msm_sig_irn[signalId],
            _ => ""
        };
    }
}
